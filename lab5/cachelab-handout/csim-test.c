    ...
    if (strcmp(op, "L") == 0) {
      ...
      if ((search_ptr = find_node(mark, &lists[index])) == NULL) {
        ...
        if (res == -1) {
          ...
          misses++;
          if (verbose) {
            printf("%lx L index:%d mark:%d misses\n", address, index, mark);
          }
        }
      } else {
        hits++;
        printf("%lx L index:%d mark:%d hits\n", address, index, mark);
      }
      ...
      if ((search_ptr = find_node(mark, &lists[index])) == NULL) {
        ...
        if (res == -1) {
          ...
          evictions++;
          if (verbose) {
            printf("%lx S index:%d mark:%d misses evictions\n", address, index,
                   mark);
          }
        } else {
          misses++;
          if (verbose) {
            printf("%lx S index:%d mark:%d misses\n", address, index, mark);
          }
        }
      } else {
        hits++;
        printf("%lx S index:%d mark:%d hits\n", address, index, mark);
      }
      ...
      if ((search_ptr = find_node(mark, &lists[index])) == NULL) {
        ...
        if (res == -1) {
          ...
          hits++;
          if (verbose) {
            printf("%lx M index:%d mark:%d misses evictions\n", address, index,
                   mark);
          }
        } else {
          misses++;
          hits++;
          if (verbose) {
            printf("%lx M index:%d mark:%d misses\n", address, index, mark);
          }
        }
      } else {
        hits++;
